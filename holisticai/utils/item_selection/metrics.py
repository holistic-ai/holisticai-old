...
def get_top_n_list_for_users(P, Q, training, topn):
    ...
    for i in range(training.shape[0]):
        ...
        sorted_ind = np.argsort(eR_new[i])[
            ::-1
        ]  # sorting the calculated ratings in the descending order for user i
        top_5 = sorted_ind[
            0:topn
        ]  # get the 5 items with highest predicted ratings # can change to top 10, so on if you want
