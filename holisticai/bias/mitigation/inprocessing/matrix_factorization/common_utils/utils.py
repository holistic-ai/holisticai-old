...
def errros(P_s, Q_s, R_s, W_s, lamda, beta):
    ...
    e_s = (lamda/2) * (pow(P_s,2) + pow(Q_s.T,2) ) + (beta/2)*(pow((pow(P_s,2) + pow(Q_s.T,2)),2)*W_s[:,None])
    countt = len(e_s)
    e  = et + np.sum(e_s)
...
def updateP(P, b, R, Q, u_rated_items, invprop):    
    P = Parallel(n_jobs = -1, verbose = 0)(delayed(update_emb)(b, 
                                                            R[u,rated_items], 
                                                            Q[:,rated_items].T, 
                                                            invprop[u,rated_items])
                                        for u, rated_items in enumerate(u_rated_items))
    return np.stack(P, axis=0)

def updateQ(P, b, R, Q, i_rated_users, invprop):   
    Q = Parallel(n_jobs = -1, verbose = 0)(delayed(update_emb)(b, 
                                                            R[rated_users, i], 
                                                            P[rated_users,:], 
                                                            invprop[rated_users,i])
