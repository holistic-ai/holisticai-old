...
class DebiasingLearningAlgorithm:
	...
	def train(self, train_observations, inv_propensities, start_vector=None):
		...
		parameters = self.optimize_debiasing_parameters(train_observations, 
                                              tempInvPropensities, 
                                              self.lamda,
									start_vec = start_vector)
		return parameters

	def optimize_debiasing_parameters(self, observed_ratings, 
		...
		serializer = ParameterSerializer(numUsers=numUsers, 
                                   numItems=numItems, 
		...
		normalized_propensities = normalize_propensities(inversePropensities, self.normalization, scale, numItems, numUsers)
		normalized_propensities = np.ma.filled(normalized_propensities, 0.0)
		observed_ratings = np.ma.filled(observed_ratings, 0)
		
		problem = Problem(observed_ratings=observed_ratings, 
		...
		ops = {'maxiter': 2000, 'disp': False, 'gtol': 1e-5,\
				'ftol': 1e-5, 'maxcor': 50, 'disp':False}

		result = scipy.optimize.minimize(fun = problem.evaluate, x0 = startVector,
