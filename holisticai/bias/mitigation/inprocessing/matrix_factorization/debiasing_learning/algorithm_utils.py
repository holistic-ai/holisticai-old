...
def process_propensities(observed_ratings, inverse_propensities):
    ...
    if inverse_propensities is None:
        inversePropensities = np.ones((numUsers, numItems), dtype = np.longdouble) * scale /\
                            numObservations
    else:
...
class Problem:
    ...
    def evaluate(self, paramVector):
        userVectors, itemVectors, userBiases, itemBiases, globalBias = self.serializer.deserialize(paramVector)
        biasedScores = predict_scores(userVectors, itemVectors, userBiases, itemBiases, globalBias, self.config['useBias'])
        ...
        if self.config['l2_regularization'] > 0:
            scaledPenalty = 1.0 * self.config['l2_regularization'] * self.config['scale'] / (self.config['numUsers'] + self.config['numItems'])
        ...
        gradient = self.serializer.serialize(userVGradient, itemVGradient, userBGradient, itemBGradient, globalBGradient)
...
def init_random_parameters(config, start_vec=None):
    ...
    return userVectorsInit, itemVectorsInit, userBiasesInit, itemBiasesInit, globalBiasInit