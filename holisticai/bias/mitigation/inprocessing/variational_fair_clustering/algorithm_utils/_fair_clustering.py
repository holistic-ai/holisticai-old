...
class FairnessUtility:
    ...
    def fit_fairness(self, X, a_p, group_prob, groups_ids):
        ...
        if (len(np.unique(l_check)) != self.K):
            l,C,S,trivial_status = self.restore_nonempty_cluster(X, self.K, self.old['l'], self.old['C'], self.old['S'])
            self.ts = self.ts+1
...
class FairClustering(FairnessUtility):
    ...
    def fit(self, X, group_prob, groups_ids, random_state):
        ...
        for i in range(self.max_iter):
            ...
            clusterE, clusterE_discrete  = self.clustering_method.compute_clustering_energy(C, l, S)
            ...
            self.monitor.save(fair_cluster_E=currentE, 
                         cluster_E=clusterE, 
                         fair_E=fairE, 
                         cluster_E_discrete=clusterE_discrete, 
                         fairness_error=fairness_error)
            
            self.logger.update(iteration=i, 
                               fairness_error=fairness_error, 
