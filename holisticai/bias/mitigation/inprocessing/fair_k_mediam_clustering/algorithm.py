...
class KMediamClusteringAlgorithm:
  def __init__(self, n_clusters=2, init_centers='Random', max_iter=1000, strategy='LS', verbose=False):
      self.k=n_clusters
  ...
  def _compute_cost(self, centers):
      ...
      for gid in self.group_ids:
        group_costs.append(np.mean(np.amin(self.distances[np.ix_(self.p_attr==gid, centers)], axis=1)))
  ...
  def _genetic_algorithm(self, X):
      ...
      from .....utils.optimizers import GeneticAlgorithm, GAHiperparameters
          
      varbound=np.array([[0,len(X)-1]]*self.k)

      ga_kargs={'max_num_iteration': self.max_iter,\
        'population_size':100,\
      ...
      optimizer = GeneticAlgorithm(function = optimization_function, 
                dimension = self.k, 
                variable_type = 'int',
