...
class PrejudiceRemover(BaseEstimator, ClassifierMixin, BMImp):
    ...
    def _build_algorithm(self):
        ...
        logger_ = PRLogger(total_iterations=self.maxiter, 
                verbose=self.verbose, 
                print_interval=self.print_interval)
                
        # Support class for objective Function
        objective_fn_ = ObjectiveFunction(estimator=self.estimator, 
                                         loss_fn=self.estimator.loss)
            
        # Algorithm class
        algorithm = PrejudiceRemoverAlgorithm(estimator=self.estimator, 
                                               logger=logger_, 
