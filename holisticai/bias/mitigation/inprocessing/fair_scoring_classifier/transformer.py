...
class FairScoreClassifier(BaseEstimator, BMImp):
    ...
    def fit(self, X, y, protected_groups, protected_labels = []):
        ...
        the same object
        """
        X, y, fairness_groups, fairness_labels = self.format_dataframe(X, y, protected_groups, protected_labels)
        self.model_ = FairScoreClassifierAlgorithm(self.objectives, fairness_groups, fairness_labels, self.constraints, self.lambda_bound, self.time_limit)
        self.model_.fit(X, y)
        return self
