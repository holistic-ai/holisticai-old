...
def fair_k_center_APPROX(dmat,p_attr,nr_centers_per_group,given_centers):
    ...
    selected_index = np.array([np.where(np.arange(n)[selection] == cluster_centersTE[ell])[0][0] for ell in np.arange(k)])
    G,centersTE = swapping_graph(selected_partition, 
                                    selected_index,
                                    selected_p_attr, 
                                    nr_centers_per_group)
    ...
    if G.size==0:
        ...
        cluster_centers_rek=fair_k_center_APPROX(dmat[np.ix_(new_data_set,new_data_set)], p_attr_newT,
                                nr_centers_per_group[G],np.arange(new_data_set.size-new_given_centers.size,new_data_set.size))
...
def swapping_graph(partition, centers, p_attr, nr_centers_per_group):
    ...
    for ell in np.arange(m):
        for zzz in np.arange(m):
            if ((CURRENT_nr_clusters_per_sex[ell]>nr_centers_per_group[ell]) and (CURRENT_nr_clusters_per_sex[zzz]<nr_centers_per_group[zzz])):
...
def heuristic_greedy_on_each_group(dmat,p_attr,nr_centers_per_group,given_centers):
    ...
    for ell in np.arange(m):
        ...
        cent_subgroup=k_center_greedy_with_given_centers(dmat[np.ix_(subgroup, subgroup)],
                                                               nr_centers_per_group[ell], given_centers_subgroup)
