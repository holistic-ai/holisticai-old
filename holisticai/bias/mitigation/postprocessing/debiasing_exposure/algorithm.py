...
class DELTRAlgorithm(object):
    ...
    def fit(self, query_ids, protected_feature, training_scores, feature_matrix):
        ...
        for q,indexes in self.indexes_per_query.items():
            self._data_per_query[hh(q, training_scores)] = find_items_per_group_per_query(training_scores.iloc[indexes], 
                                                                                                protected_feature.iloc[indexes])
            
            self._data_per_query[hh(q, feature_matrix)] = find_items_per_group_per_query(feature_matrix.iloc[indexes], 
                                                                                                protected_feature.iloc[indexes])
    ...
    def _calculate_cost(self, training_judgments, predictions, prot_idx, data_per_query_predicted):
        ...
        loss_exposure = sum([exposure_diff(predictions[indexes], prot_idx[indexes]) \
                                for indexes in self.indexes_per_query.values()])

        return np.asarray(results), loss_standard, loss_exposure

    def _loss(self, which_query, training_judgments, predictions, prot_idx, data_per_query_predicted):
        """ Calculate loss for a given query """
        
        result = -np.dot(np.transpose(topp(self._data_per_query[(hh(which_query,training_judgments))][0])),
                    np.log(topp(data_per_query_predicted[hh(which_query,predictions)][0]))) / np.log(predictions.size)
    ...
    def _calculate_gradient(self, predictions):
        ...
        args_list = (self.feature_data_per_query_list, 
                     predictions_per_query_list, 
                     self.prot_idx_per_query_list, 
                     self.true_data_per_query_list)
    ...
    def _grad(self, predictions_size, 
        ...
        if not self._no_exposure:
            result = result.reshape(-1) + self._gamma * 2 * exposure_diff(predictions_per_query, prot_idx_per_query) \
                      * self._normalized_topp_prot_deriv_per_group_diff(feature_data_per_query, predictions_per_query, 
                                                                        prot_idx_per_query)
        return result

    def _normalized_topp_prot_deriv_per_group_diff(self, training_features_per_query, predictions_per_query, prot_idx_per_query):
        """
        Description
        -----------
