...
def exposure_metric(rankings, group_col: str, query_col: str, score_col: str):
    ...
    exp_diff = np.mean([exposure_diff(ranking[score_col], ranking[group_col]) for q,ranking in rankings.groupby(query_col)])
    
    metric_values = [exposure_ratio(ranking[score_col], ranking[group_col]) for q,ranking in rankings.groupby(query_col)]
...
def find_items_per_group_per_query(data, protected_feature):
    ...
    if np.any(np.isnan(data_per_query)):
        raise Exception("data has NaN values!!, fix your data or normalize it before training.")
