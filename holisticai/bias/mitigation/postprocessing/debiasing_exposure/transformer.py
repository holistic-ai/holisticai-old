...
class DebiasingExposure:
    ...
    def __init__(self, 
        ...
        self.algorithm = DELTRAlgorithm(self._gamma, 
    ...
    def fit(self, rankings: pd.DataFrame):
        ...
        if self.feature_cols == []:
            ...
            self.feature_cols = [col for col in rankings.columns.to_list() if not (col in restricted_cols)]
        
        # prepare data
        query_ids, doc_ids, protected_feature, feature_matrix, training_scores = self._prepare_data(rankings, has_judgment=True)
    ...
    def transform(self, rankings: pd.DataFrame):
        ...
        result = pd.DataFrame({ self.query_col: query_ids,
