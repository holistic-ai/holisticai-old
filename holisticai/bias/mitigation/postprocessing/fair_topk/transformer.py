...
from .algorithm_utils.valitation_utils import validate_basic_parameters,check_ranking
...
class FairTopK(BMPost):
    ...
    def transform(self, rankings,  p_attr = None):
        ...
        re_rankings = [df if self.is_fair(df) else self.transform_ranking(df) \
    ...
    def _fair_top_k(self, protected_candidates, non_protected_candidates, mtable):
        ...
        for i in range(self.top_n):
            ...
            if idxProtected >= len(protected_candidates):
                ...
                if protected_candidates.iloc[idxProtected][self.score_col] >= non_protected_candidates.iloc[idxNonProtected][self.score_col]:
