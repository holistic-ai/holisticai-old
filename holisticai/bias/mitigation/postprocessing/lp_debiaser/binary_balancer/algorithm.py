...
class BinaryBalancerAlgorithm:
    ...
    def fit(self, y_true, group_num, y_pred=None, y_proba=None):
        ...
        assert not ((y_pred is None) and (y_proba is None)), f"y_pred or y_proba must be passed"
        ...
        if y_proba is not None:
            ...
            self.__roc_stats = [tools.loss_from_roc(y_true[ids], 
                                                    probs[ids], 
                                                    self.rocs[i]) 
        ...
        rates_by_group = [tools.CLFRates(y_true[i], y_pred[i]) 
                         for i in group_ids]
        ...
        dr = [(g.nr * p[i], g.pr * p[i]) 
                for i, g in enumerate(rates_by_group)]
        ...
        self.y_adj = tools.pred_from_pya(y_pred=y_pred, 
                                         group_num=group_num,
                                         pya=self.pya, 
                                         binom=self.binom)
