...
class WasserteinBarycenterAlgorithm:
    ...
    def fit(self, y_pred: np.ndarray, sensitive_groups: np.ndarray):
        ...
        group_num = self.sens_groups.fit_transform(sensitive_groups, convert_numeric=True).squeeze()
        self.group_values = np.unique(group_num)
                
        group_freq = [np.sum(group_num==self.group_values[0]) , np.sum(group_num==self.group_values[1])]
