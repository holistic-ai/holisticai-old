...
def f_mincon(etaX, S, M, L, beta, lambda_):
    ...
    def h_fn(E, S, i): return 2*p[S]*i*M*E/L - p[S]*(i**2)*(M**2)/(L**2)

    def tot_fn(H, S, i): return np.exp(lambda_[i+L]*(2*S-1)/beta  + H/beta)
    ...
    for i in np.arange(2*L+1):
        H_i = Hs[i]
        tmp1 = TOTs[i]/TOT
        RIS_i = tmp1*((2*S-1)*lambda_[i] + H_i - beta*np.log((2*L+1)*tmp1))
        RISs.append(RIS_i)
