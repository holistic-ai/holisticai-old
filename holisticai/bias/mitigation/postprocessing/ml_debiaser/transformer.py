...
class MLDebiaser(BMPost):
    ...
    def fit(self, y_proba: np.ndarray, group_a: np.ndarray, group_b: np.ndarray):
        ...
        if num_classes>2:
            self.algorithm = Reduce2BinaryAlgorithm(gamma=self.gamma, eps=self.eps, eta=self.eta, 
                                            num_classes=num_classes, sgd_steps=self.sgd_steps, 
                                            full_gradient_epochs=self.full_gradient_epochs, 
                                            batch_size=self.batch_size, max_iter=self.max_iter,
                                            verbose=self.verbose)
        else:
            self.algorithm = RandomizedThresholdAlgorithm(gamma=self.gamma, eps=self.eps,
